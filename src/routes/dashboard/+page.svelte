<script>
  ...
  onMount(async () => {
    try {
      ...
      if (stats.error) { error = stats.error; loading = false; return; }
      loading = false;
    ...
    new Chart(cweCanvas, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: stats.cwe_counts.map(c => SEVERITY_COLORS[c.severity] ?? "#6b7280"),
          borderRadius: 4,
        }]
      },
      options: {
  ...
  function drawConfidence() {
    ...
    new Chart(confidenceCanvas, {
      ...
      data: {
        ...
        datasets: [{
          label: "Predictions",
          data: [bins.bin_0_50 ?? 0, bins.bin_50_70 ?? 0, bins.bin_70_90 ?? 0, bins.bin_90_100 ?? 0],
...
<div class="bg-gray-950 text-white px-6 py-8 min-h-screen">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-500 text-sm mt-1">Aggregate stats across all analyses</p>
    </div>
    ...
    {:else}
      ...
      <div class="grid grid-cols-4 gap-4 mb-6">
        {#each [
          { label: "Analyses Run",      value: stats.kpis.total_analyses   ?? 0, color: "text-cyan-400"  },
          { label: "Functions Scanned", value: stats.kpis.total_functions  ?? 0, color: "text-blue-400"  },
          { label: "Vulnerable",        value: stats.kpis.total_vulnerable ?? 0, color: "text-red-400"   },
          { label: "Clean",             value: stats.kpis.total_safe       ?? 0, color: "text-green-400" },
        ] as kpi}
          <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <p class="text-gray-500 text-xs uppercase tracking-wide mb-1">{kpi.label}</p>
      ...
      <div class="grid grid-cols-3 gap-4 mb-4">
        ...
        <div class="col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-5">
          <p class="text-sm font-semibold text-gray-300 mb-4">Vulnerability Breakdown by CWE</p>
          {#if stats.cwe_counts.length}
            <canvas bind:this={cweCanvas}></canvas>
          {:else}
            <p class="text-gray-600 text-sm text-center py-8">No vulnerable functions yet.</p>
          {/if}
        </div>

        <div class="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col">
          <p class="text-sm font-semibold text-gray-300 mb-4">Severity Distribution</p>
          {#if stats.severity_counts.length}
            <div class="relative flex-1 flex items-center justify-center">
              <canvas bind:this={severityCanvas}></canvas>
              <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div class="text-center">
                  <p class="text-2xl font-bold text-white">{stats.kpis.total_vulnerable ?? 0}</p>
                  <p class="text-xs text-gray-500">vulnerable</p>
                </div>
              </div>
            </div>
          {:else}
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <p class="text-sm font-semibold text-gray-300 mb-4">Vulnerable vs Safe per File</p>
          {#if stats.file_ratios.length}
            <canvas bind:this={fileCanvas}></canvas>
          {:else}
            <p class="text-gray-600 text-sm text-center py-8">No files scanned yet.</p>
          {/if}
        </div>

        <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <p class="text-sm font-semibold text-gray-300 mb-1">Model Confidence Distribution</p>
          <p class="text-xs text-gray-600 mb-4">Low confidence = needs human review</p>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-800">
          <p class="text-sm font-semibold text-gray-300">Recent Analyses</p>
        </div>
          ...
          <tbody>
            {#each stats.recent_analyses as item}
              <tr class="border-b border-gray-800 hover:bg-gray-800 transition-colors">
                <td class="px-5 py-3 font-mono text-white">{item.project_name}</td>
                <td class="px-5 py-3 text-gray-400">{item.timestamp}</td>
                <td class="px-5 py-3 text-gray-300">{item.total_functions ?? 0}</td>
                <td class="px-5 py-3">
                ...
                <td class="px-5 py-3 text-right">
                  <a href="/report/{item.id}" class="text-cyan-400 hover:text-cyan-300">View →</a>
                </td>
              </tr>
            {/each}
          </tbody>
